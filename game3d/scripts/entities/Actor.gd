...
func apply_slow(multiplier: float, duration: float) -> void:
	var new_multiplier: float = clamp(multiplier, 0.35, 1.0)
	var new_duration: float = max(0.1, duration)
...
func splinter_tag() -> String:
	if not splinter_active:
		return ""
	return "[SPLINTER]"
...
func set_splinter_state(active: bool, next_label: String = "") -> void:
	splinter_active = active
	if not splinter_active:
		splinter_label = ""
		_refresh_control_visual()
		return
	splinter_label = next_label
	_refresh_control_visual()
...
func _refresh_control_visual() -> void:
	...
	if slow_time_left > 0.0:
		...
		if splinter_active:
			var splinter_glow := _splinter_glow_color()
			if material.emission_enabled:
				material.emission = material.emission.lerp(splinter_glow, 0.15)
			else:
				material.emission_enabled = true
...
func _splinter_glow_color() -> Color:
	if faction == "human":
...
func _report_death_if_needed(game_loop: GameLoop) -> void:
	if not is_dead or death_reported:
		return

	death_reported = true

	var reason: String = death_reason if death_reason != "" else "unknown"
	var safe_attacker: Actor = null

	if is_instance_valid(last_attacker) and last_attacker != self and not last_attacker.is_dead:
		safe_attacker = last_attacker

	if game_loop != null:
		game_loop.register_death(self, safe_attacker, reason)
