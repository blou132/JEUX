...
SCRIPT = ROOT / "tools" / "export_faction_templates.py"


class FactionTemplatesExportToolTests(unittest.TestCase):
    def test_export_writes_expected_templates(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "factions.json"
            ...
            ids = {template["id"] for template in factions}
            self.assertEqual(ids, {"human_core", "monster_core"})
