...
class CollectSupportMetricsRuntimeToolTests(unittest.TestCase):
    ...
    def test_trace_export_with_simulated_trace_file_returns_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("payload_has_support_gate: yes", result.stdout)
            self.assertIn("payload_has_champion_support: yes", result.stdout)
            self.assertIn("payload_has_champion_resolution: yes", result.stdout)
            self.assertIn("missing_payload_fields: none", result.stdout)
