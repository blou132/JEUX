...
- Analyse CSV alignee sur les syntheses runtime/multi (traits et impacts `density_preference`, `gregariousness`, `competition_tolerance`, `resource_commitment` conserves en CSV).
...
- Variabilite individuelle legere de tolerance a la competition locale pour la nourriture (`competition_tolerance`) heritable, mutante et visible en stats/synthese/debug.
- Variabilite individuelle legere de commitment ressource locale (`resource_commitment`) heritable, mutante et visible en stats/synthese/debug.
...
- Evaluation legere de l'impact `resource_commitment` (moyenne/dispersion, frequences `stay`/`switch`, biais d'usage, effet distance a l'ancre et effet memoire de rappel) visible en stats, synthese run, multi-runs, export et analyse.
...
### Variabilite individuelle de tolerance environnementale
- Trait leger ajoute: `environmental_tolerance`.
- Effet volontairement limite:
  - module legerement le drain energetique observe selon la zone locale de fertilite (`poor`/`rich`).
  - effet plus marque en zone pauvre, sans ajouter de biome complexe ni nouvelle mecanique lourde.
- Heredite simple + mutation legere via le pipeline genetique existant.
- Observation dans les logs/syntheses:
  - `dynamique_*`: `traits_comp_moy` (`env`), `traits_disp` (`env_sigma`) et `env_tick` (`zone_mult`, `poor_evt`, `rich_evt`, `poor_mu`, `rich_mu`, `poor_bias`, `rich_bias`).
  - `Run Summary` / `Multi-Run Summary`: `traits_moy` / `traits_finaux_moy` (`env`) et `traits_impact` / `traits_impact_moy` (`env_mu`, `env_sigma`, `env_obs*`).

### Variabilite individuelle de timing reproductif
...
- Effet volontairement limite:
  - `mobility_efficiency > 1.0`: deplacement effectif legerement plus long par action.
  - `mobility_efficiency < 1.0`: deplacement effectif legerement plus court.
- L'effet est applique aux deplacements existants (cible nourriture, errance/recherche, memoire, suivi social, fuite) sans nouvelle mecanique de locomotion.
- Heredite simple + mutation legere via le pipeline genetique existant.
- Observation dans les logs/syntheses:
...
- Quand les metriques existent, la synthese inclut aussi `longevity_factor_batch`:
  ...
  - configuration la plus stable (regle batch standard)
  - signalement explicite des cas ambigus ou insuffisants
- Quand les metriques existent, la synthese inclut aussi `environmental_tolerance_batch`:
  - configuration qui maximise l'effet observe en zone pauvre (proxy: `abs(environmental_tolerance_poor_drain_bias)`)
  - configuration qui maximise l'effet observe en zone riche (proxy: `abs(environmental_tolerance_rich_drain_bias)`)
...
- Quand les metriques existent, la synthese inclut aussi `mobility_efficiency_batch`:
  ...
  - configuration la plus stable (regle batch standard)
  - signalement explicite des cas ambigus ou insuffisants
- Quand les metriques existent, la synthese inclut aussi `stress_tolerance_batch`:
  - configuration qui maximise l'effet observe sous pression (proxy: `abs(stress_tolerance_pressure_mean - 1)`)
  - configuration qui maximise la modulation de fuite en situation tendue (proxy: `abs(stress_tolerance_pressure_flee_bias)`)
...
configuration_plus_stable: mutation_variation=0.1 (taux_ext=0.00, pop_finale_moy=42.50, gen_max_moy=2.50)
environmental_tolerance_batch:
effet_zone_pauvre_max: mutation_variation=0.2 (impact_abs_moy=0.120)
...
configuration_plus_stable: mutation_variation=0.1 (taux_ext=0.00, pop_finale_moy=42.50, gen_max_moy=2.50)
stress_tolerance_batch:
effet_sous_pression_max: mutation_variation=0.1 (impact_abs_moy=0.090)
...
py -m unittest tests.test_resource_commitment_trait
```

## Lire les logs debug (indicateurs utiles)
Chaque bloc de log periodique contient:
- ligne principale: `population`, `vivants/morts`, `nourriture`, `energie_moy`, `age_moy`, `gen_moy`, naissances/deces, moyennes de traits (`prudence`, `dominance`, `risk_taking`, `stress_tolerance`, `repro_drive`, `memory_focus`, `social_sensitivity`, `behavior_persistence`, `exploration_bias`, `density_preference`, `gregariousness`, `competition_tolerance`, `mobility_efficiency`, `energy_efficiency`, `exhaustion_resistance`, `longevity_factor`, `environmental_tolerance`, `reproduction_timing`, `hunger_sensitivity`, `resource_commitment`).
...
- `traits_comp_moy` / `traits_disp` dans `dynamique_*`: inclut aussi `fp`/`tp` (moyennes `food_perception`/`threat_perception`) et `fp_sigma`/`tp_sigma`, `rk`/`rk_sigma` pour la prise de risque, `st`/`st_sigma` pour la tolerance au stress, `bp`/`bp_sigma` pour la persistance comportementale, `ex`/`ex_sigma` pour l'exploration spatiale, `dp`/`dp_sigma` pour la preference de densite locale, `gr`/`gr_sigma` pour la gregarite, `ct`/`ct_sigma` pour la tolerance a la competition locale, `mo`/`mo_sigma` pour la mobilite, `ee`/`er` et `ee_sigma`/`er_sigma` pour l'endurance energetique, `lg`/`lg_sigma` pour la longevite, `env`/`env_sigma` pour la tolerance environnementale, `rt`/`rt_sigma` pour le timing reproductif, ainsi que `hs`/`hs_sigma` pour la sensibilite a la faim.
...
- `resource_commitment_log` / `resource_commitment_tick` dans `dynamique_*`: usage observe de `resource_commitment` (`guides`, `stay`, `switch`, parts/frequences, `stay_mu`/`switch_mu`, biais `rc`, effet `delta_ancre_rc`, `mem_bias_rc`, `recall_rc`).
...
- `competition_log` / `competition_tick` dans `dynamique_*`: usage observe de `competition_tolerance` (`guides`, `stay`, `avoid`, parts/frequences, `stay_mu`/`avoid_mu`, biais `ct`/`stay`/`avoid`, `ct_voisins`, `delta_ancre_ct`).
