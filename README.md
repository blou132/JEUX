...
- l'analyse expose aussi un bloc global `support_systems_summary` pour lire `support_gate` et `rally_champion` ensemble:
...
- integration CI optionnelle (workflow):
  ...
  - le script CI est fail-safe par defaut: en cas d'erreur technique d'analyse, il produit un warning dans le Summary et un rapport minimal (`Support metrics CI check failed`, `reason: analysis failed`, `blocking: no`).
...
- Smoke report vs runtime report:
...
- v249 gameplay/debug note: `v249 adds a controlled rally_champion target availability fallback only. Gameplay rules are unchanged.`
